var Global = {
	...
	spinresult: function() {
		...
		if (Math.random() < 0.5 + Math.cos(p * Math.PI) * 0.475 && this.spinnum > this.mininterval) {
			...
			if (x > this.probability[5]) {
				...
				if (x > this.probability[4] && this.mincost[0] > 30) {
					//一般 双水果
					this.mincost[0] = 0;
					...
					if (x > this.probability[3] && this.mincost[1] >= 100) {
						//幸运 三水果
						this.mincost[1] = 0;
						r[0] = r[1] = r[2] = Math.round(Math.random() * 5);
						r[3] = 21;
					} else {
						if (x > this.probability[2] && this.mincost[2] >= 200) {
							//特殊 双水果+ 小丑
							...
							if (x > this.probability[2] && this.mincost[3] >= 350) {
								// 头奖 1小丑
								this.mincost[3] = 0;
								r[0] = 6;
	...
	spinresult_B: function() {
		...
		if (p > 0.9) {
			...
			if (this.spinnum > this.maxinterval) this.addition -= 0.1;
		} else {
		}
	},
};
